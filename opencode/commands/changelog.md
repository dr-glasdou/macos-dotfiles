...
2. Bump the project version to match X.Y.Z in the relevant manifest:
   ...
   - `pubspec.yaml` (Flutter) → set `version: X.Y.Z+N`, always incrementing the build number `+N` by 1 if the version is the same, reset to 1 if the version is different.
